...
export class SnarkExecuteContext implements ExecutionContext {
    ...
    async execute(transitionName: string, params: string[]): Promise<ExecutionOutput> {
        ...
        const cmd = `cd ${this.config.contractPath} && snarkos developer execute ${this.config.appName}.aleo ${transitionName} ${formattedParams} --private-key ${this.config.privateKey} --network ${this.config.networkMode} --query ${nodeEndPoint} --dry-run`;
        console.log(cmd);
        const { stdout } = await execute(cmd);
        console.log(stdout);
        const { transaction } = this.parser.parse(stdout);
        if (!transaction) throw new Error('Failed to execute transaction');
        await broadcastTransaction(transaction!, nodeEndPoint!, this.config.networkName!);
